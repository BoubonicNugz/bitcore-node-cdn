...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.bitcore-cdn');
  var defaultConfigFile = path.resolve(defaultPath, './bitcore-node-cdn.json');

  if (!fs.existsSync(defaultPath)) {
    mkdirp.sync(defaultPath);
  }

  var defaultServices = ['canadaecoind', 'web'];
