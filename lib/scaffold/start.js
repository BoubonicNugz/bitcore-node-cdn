...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for canadaecoind is necessary for this upgrade with the "reindex=1" canadaecoin.conf option. \n' +
                 'There are changes necessary in both canadaecoin.conf and bitcore-node-cdn.json. \n\n' +
        ...
        servicesConfig: {
          bitcoind: {
            spawn: {
              ...
              exec: path.resolve(__dirname, '../../bin/canadaecoind')
            }
          }
        }
      };
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './bitcore-node-cdn.json');
  ...
  node.on('ready', function() {
    log.info('Bitcore CDN Node ready');
  });
