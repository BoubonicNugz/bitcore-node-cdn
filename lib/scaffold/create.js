...
var BASE_PACKAGE = {
  description: 'A full Canada eCoin Node build with Bitcore',
  repository: 'https://github.com/boubonicnugz/bitcore-node-cdn/',
  license: 'MIT',
  readme: 'README.md',
  dependencies: {
    'bitcore-lib': '^' + bitcore.version,
    'bitcore-node-cdn': version
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/bitcore-node-cdn.json', configJSON);
      if (!isGlobal) {
